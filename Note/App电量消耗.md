## 写在前面的话
我们在做优化相关的时候，除了卡顿问题另一个关注的大点就是耗电优化，特别是有时候会发现“某个页面的前一个版本还好好的，这个版本的耗电怎么多了那么多”，那么你首先想到可能就是这个页面有没有开启定位，网络请求是不是频繁，亦或是定时任务时间是不是间隔过小。接下来，你会去查找耗电问题到底是怎么引起的。你去翻代码的时候却发现，这个页面的相关功能在好几个版本中都没改过了。那么，到底是什么原因使得这一个版本的耗电量突然增加呢？不如就使用排除法吧，你把功能一个个都注释掉，却发现耗电量还是没有减少，那么到底该怎么办呢？
...
> 不过，现在版本的IOKit Framework在iOS项目已经无法导入了，只能新建一个Mac项目然后考进去。
...
## 如何诊断电量问题
到了这里，电量已经获取了，我们继续回到最初的问题，如果通过排除法把所有功能注释之后发现还有问题，那么这个耗电一定是由其他线程引起的，这其实也是最坑的地方，创建这个耗电线程的地方可能是在其他地方，比如是由第三方库引起，或者是公司其他团队开发的二方库。
所以，这个时候需要逆向地去思考这个问题，也就是说，我们还是先反过来看看出现电量问题的期间，哪个线程是有问题的。通过下面的这段代码，你就可以获取到所有线程的信息：
...
```
有了这个 cpu_usage 字段，你就可以通过遍历所有线程，去查看是哪个线程的 CPU 使用百分比过高了。如果某个线程的 CPU 使用率长时间都比较高的话，比如超过了 90%，就能够推断出它是有问题的。这时，将其方法堆栈记录下来，你就可以知道到底是哪段代码让你 App 的电量消耗多了。通过这种方法，你就可以快速定位到问题，有针对性地进行代码优化。多线程 CPU 使用率检查的完整代码如下：
...
CPU 和 I/O 这两大耗电问题都解决后，还有什么要注意的呢？这里还有两份关于 App 电量消耗的资料，有兴趣的同学可以看一下：

1. 苹果公司专门维护的一个电量优化指南：[Energy Efficiency and the User Experience](https://developer.apple.com/library/archive/documentation/Performance/Conceptual/EnergyGuide-iOS/)
2. 苹果公司在 2017 年 WWDC 的 Session 238 也分享了一个关于如何编写节能 App 的主题：[Writing Energy Efficient Apps](https://developer.apple.com/videos/play/wwdc2017/238/)
