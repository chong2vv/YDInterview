...
如果是swift的话，我们可以使用Swift Playground来做快速调试，但是对于OC来说却没有办法直接使用，那么有没有适合于OC的库呢？答案肯定是有的——**[Injection for Xcode](https://github.com/johnno1962/InjectionIII)**

他是由John Holdsworth 开发了一个叫作 Injection 的工具可以动态地将 Swift 或 Objective-C 的代码在已运行的程序中执行，以加快调试速度，同时保证程序不用重启。
...
```
然后在这个方法中添加一个断点，按下 ctrl + = ，接下来你会发现程序运行时会停到断点处，这样你的代码就成功地被运行中的 App 执行了。那么，Injection 是怎么做到的呢？Injection 会监听源代码文件的变化，如果文件被改动了，Injection Server 就会执行 rebuildClass 重新进行编译、打包成动态库，也就是 .dylib 文件。编译、打包成动态库后使用 writeSting 方法通过 Socket 通知运行的 App。writeString 的代码如下：
...
这里主要介绍3款主流的静态分析器OCLint、Clang 静态分析器、Infer。

### OCLint 
OCLint 是基于 Clang Tooling 开发的静态分析工具，主要用来发现编译器检查不到的那些潜在的关键技术问题，包含了很多的规则，这些规则已经基本覆盖了具有通用性的规则，主要包括语法上的基础规则、Cocoa 库相关规则、一些约定俗成的规则、各种空语句检查、是否按新语法改写的检查、命名上长变量名短变量名检查、无用的语句变量和参数的检查。除此之外，还包括了和代码量大小是否合理相关的一些规则，比如过大的类、类里方法是否太多、参数是否过多、Block 嵌套是否太深、方法里代码是否过多、圈复杂度的检查等。

OCLint的安装方式可以直接使用HomeBrew安装：
...
### Clang静态分析器
[Clang 静态分析器](http://clang-analyzer.llvm.org/release_notes.html)（Clang Static Analyzer）是一个用 C++ 开发的，用来分析 C、C++ 和 Objective-C 的开源工具，是 Clang 项目的一部分，构建在 Clang 和 LLVM 之上。Clang 静态分析器的分析引擎用的就是 Clang 的库。
...
```
关于他的更多使用方法有兴趣可以查阅一下[文档](http://clang-analyzer.llvm.org/scan-build)。
...
checker 的官方示例代码里有一个非常实用的，也就是内存泄露检查示例 MallocChecker，你可以点击这个[链接](http://clang.llvm.org/doxygen/MallocChecker_8cpp_source.html)查看代码。
...
路径敏感分析也包含了模拟内存管理，SymbolManager 符号管理里维护着变量的生命周期分析。
...
```
brew install autoconf automake cmake opam pkg-config sqlite gmp mpfrbrew cask install java
```
...
其实这三个库都是基于Clang库开发的，个人的话比较喜欢使用Infer，原因就是使用比较简单，但是Inter虽然效率高，但是可定制性比较弱，所以比较中等。Clang静态分析器和Xcode集成度高，也支持命令行，不过规则少，只能检查出大问题，所以偏重于可用性。至于OCLint则是可定制程度过高了，所以很容易找不到重点，个人不是很喜欢使用。
